...
export const SpecialAttacks: React.FC<SpecialAttacksProps> = ({
  ...
  const handleMouseDown = (e: React.MouseEvent, attackId: string) => {
    ...
    if (!attack || attack.currentCooldown > 0) return;
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      // Always allow placement anywhere on screen, convert to canvas coordinates
      if (canvasRect) {
        ...
        // Fallback to center if no canvas rect
      ...
      >
        {specialAttacks.map((attack) => {
          ...
          return (
            <div
              key={attack.id}
              className={`relative w-16 h-16 rounded-full border-2 border-accent bg-card/95 backdrop-blur-sm shadow-lg flex items-center justify-center cursor-pointer transition-all hover:scale-110 ${
                canUse ? 'hover:bg-accent/20' : 'opacity-50 cursor-not-allowed'
              }`}
