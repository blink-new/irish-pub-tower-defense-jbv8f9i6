...
export const GameBoard: React.FC<GameBoardProps> = ({
  ...
  useEffect(() => {
    ...
    // Create smoke systems for various chimneys
  ...
  const drawSpecialAttackSplashes = useCallback((ctx: CanvasRenderingContext2D) => {
    specialAttackEffects.forEach(effect => {
      console.log(`🎯 SPECIAL ATTACK DEBUG - Drawing effect at: (${effect.x}, ${effect.y}), type: ${effect.type}`);
  ...
  const draw = useCallback((currentTime?: number) => {
    ...
    // Draw natural tree-line border with darker grass gradient
    ...
    if (gameState.placingTowerType) {
      ...
      // Highlight available placement pads
  ...
  }, [gameState, drawPath, drawTowers, drawEnemies, drawProjectiles, particleSystem, drawSpecialAttackSplashes, flashMessage]);
  ...
  const handleCanvasClick = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (gameState.placingTowerType) {
      // Find nearest placement pad
      const nearestPad = findNearestPlacementPad({ x, y });
      
      if (nearestPad) {
        // Check if the placement pad is valid (not occupied)
        const occupiedPositions = gameState.towers.map(tower => tower.position);
        const isValid = isValidPlacementPosition(nearestPad, occupiedPositions);
